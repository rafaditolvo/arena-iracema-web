...
const S3_BUCKET = process.env.S3_BUCKET ?? "arenairacemalandpage";
const S3_BUCKET_PLANS = process.env.S3_BUCKET_PLANS ?? "saveform";
const DYNAMODB_TABLE_USERS = process.env.DYNAMODB_TABLE_USERS ?? "landpage_users";
...
const mountCSV= async (bodyList) => {
    const header = `Nome;Email;Telefone;Data de nascimento`;
    const body = bodyList.map((body) => {
      const { firstname, email, phone, nasc } = JSON.parse(body.toString('utf-8'));
      return `"${firstname}";"${email}";"${phone}";"${nasc}";`;
    }).join('\n');
